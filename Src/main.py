...
# import sys
# Project_root = '/home/shaozc/Project/Kaggle-PANDA/Kaggle-PANDA-Solution'
# sys.path.append(Path(Project_root) / Path('//Utils'))
# sys.path.append(Path(Project_root) / Path('/factory'))
# sys.path.append(Path(Project_root) / Path('/LightningModule'))
# from factory import read_yaml
# from LightningModule import LightningModuleReg,LitModel
# from Utils import seed_torch, src_backup, Logger
...
def make_parse():
    ...
    arg("--config", default="/home/shaozc/Project/Kaggle-PANDA/Kaggle-PANDA-Solution/Configs/3.yaml", type=str, help="config path")
...
def make_output_path(output_path: Path, debug: bool) -> Path:
    if debug:
        ...
        output_path = Path("/newdata/shaozc/Kaggle-HuBMAP/output/tmp") / name_tmp
        cnt = 0
...
def train_a_kfold(cfg, cfg_name: str, output_path: Path) -> None:
    ...
    if server == 'kaggle':
        ...
        # model_dir = ['/newdata/shaozc/Kaggle-HuBMAP/output/model/2_CustomEnResNet34_kfold_1_bestloss.ckpt']
        for path in model_dir:
            new_model = model.load_from_checkpoint(checkpoint_path=path, 
                                                # hparams_file='/newdata/shaozc/Kaggle-HuBMAP/output/model/1/1.yaml',
                                                cfg=cfg)
...
def main():
    ...
    # Make output path
    output_path = Path("/newdata/shaozc/Kaggle-HuBMAP/output/model/%s"%Path(args.config).stem)
    ...
    src_backup(input_dir=Path("/home/shaozc/Project/Kaggle-PANDA/Kaggle-PANDA-Solution/"), output_dir=src_backup_path)
