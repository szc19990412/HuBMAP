...
#-------Init部分-------#
    def __init__(self, cfg):
        ...
        # self.valid_label = []
        #----
        self.watch_train = self.cfg.Data.testdataset.watch_train
...
#-----------------training_step----------#
    ...
    def training_step(self, batch, batch_nb):
        ...
        #-----分类加分割loss
        # loss = 0.5*symmetric_lovasz(logit, mask) + 0.5*criterion_binary_cross_entropy(x_class, label)
        #-----分割loss
...
#----------------validation_step-------------#
    def validation_step(self, batch, batch_nb):
        ...
        # x_class = x_class.detach().round()
        # label = label.detach().round()

        # self.valid_xclass.append(x_class.data.cpu().numpy())
        # self.valid_label.append(label.data.cpu().numpy())

        self.valid_probability.append(logit.data.cpu().numpy())
        self.valid_mask.append(mask.data.cpu().numpy())

    def validation_epoch_end(self, outputs):
        # x_class = np.concatenate(self.valid_xclass)
        # label = np.concatenate(self.valid_label)
        # # Accuracy and Kappa score
        # v_kappa = quadratic_weighted_kappa(x_class, label)
        # acc = (x_class == label).mean() * 100.0
        # self.log('acc', acc, prog_bar=True)
        # self.log('v_kappa', v_kappa, prog_bar=True)
        
        probability = np.concatenate(self.valid_probability)
        mask = np.concatenate(self.valid_mask)#连起来
        loss = 0.5*np_binary_cross_entropy_loss(probability, mask) + 0.5*np_binary_cross_entropy_loss(x_class, label)
...
#-------------------test_step-----------------------#
    ...
    def test_step(self, batch, batch_nb):
        ...
        if self.watch_train:
            image, mask, structure, ids = batch #此处取出的类型是tensor
            mask = mask.cpu().numpy().squeeze(0)
            submit_dir = Path(self.submit_dir) / str(self.kfold) / '_train'
        ...
        tile_probability = []
        batch = np.array_split(tile_image, len(tile_image)//2) #切分，64份
        for t,m in enumerate(batch):
            m = torch.from_numpy(m).cuda()
            
            p = []
            logit, xclass = self(m)
            xclass = torch.sigmoid(xclass) 
            xclass = xclass.data.cpu().numpy().round()
            if xclass == 0:
                logit = np.zeros_like(logit)
            p.append(torch.sigmoid(logit))

            #----
            if self.tta: #tta
                logit, xclass = self(m.flip(dims=(2,)))
                xclass = torch.sigmoid(xclass) 
                xclass = xclass.data.cpu().numpy().round()
                if xclass == 0:
                    logit = np.zeros_like(logit)
                ...
                logit, xclass = self(m.flip(dims=(3,)))
                xclass = torch.sigmoid(xclass) 
                xclass = xclass.data.cpu().numpy().round()
                if xclass == 0:
                    logit = np.zeros_like(logit)
        ...
        predict = (probability>0.3).astype(np.float32)
...
#--------------------DataLoader-------------------------#
    ...
    def get_loader(self, phase):
        ...
        # 数据集中，每一类的数目。
        if phase == 'train':        
            class_sample_counts = [dataset.get_classes_zero(), dataset.get_classes_one()]
        ...
        return DataLoader(
            ...
            sampler = sampler if phase == "train" else SequentialSampler(dataset),
