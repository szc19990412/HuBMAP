...
class TestDataset(Dataset):
    ...
    def __getitem__(self,idx): #返回一个tiff
        ...
        ids = '54f2eec69'
        if self.watch_train:
            image = read_tiff('/data/shaozc/Kaggle-HuBMAP/train/' + '%s.tiff'%(ids))
            height, width = image.shape[:2]
            df_train = pd.read_csv('/data/shaozc/Kaggle-HuBMAP/' + 'train.csv')
            taridx = df_train[df_train['id'] == ids].index.values[0] #取出行坐标
            encoding = df_train.iloc[taridx]['encoding']
