...
def make_parse():
    ...
    arg("--tile-size", type=int, default=320)
    arg("--tile-average-step", type=int, default=160)
    arg("--data-dir", type=str, default='/data/shaozc/Kaggle-HuBMAP')
    arg("--train-tile-dir", type=str, default=f'/data/shaozc/Kaggle-HuBMAP/0.25_320_160_train_corrected')
...
def run_make_train_tile():
    ...
    for i in tqdm(range(0,len(df_train))):
        ...
        if id == 'e79de561c':
            y_shift = -6
            x_shift = -44
            mask = global_shift_mask(mask,y_shift,x_shift)
        if id == '54f2eec69':
            y_shift = 10
            x_shift = 15
            mask = global_shift_mask(mask,y_shift,x_shift)
